...
cat "$OUTPUT_DIR/wayback.txt" "$OUTPUT_DIR/gau.txt" | sort -u > "$OUTPUT_DIR/allurls.txt"
...
cat "$OUTPUT_DIR/allurls.txt" | gf sqli | tee "$OUTPUT_DIR/sqli_urls.txt" | while read -r url; do
...
try:
    ...
    print(f'[!] Error during CSRF/Clickjack check: {e}')
EOF
...
if command -v corsy &> /dev/null; then
    ...
    echo "[!] Corsy not found. Skipping CORS check."
fi
...
if [ -s "$OUTPUT_DIR/live.txt" ]; then
    ...
    echo "[!] No live hosts found to test for cache deception."
fi
...
if [ -s "$OUTPUT_DIR/live.txt" ]; then
    ...
    echo "[!] No live hosts found to test host header attacks."
fi

### 14. GraphQL Enumeration ###
echo "[*] Checking for GraphQL endpoints..."
grep -i "graphql" "$OUTPUT_DIR/allurls.txt" | tee "$OUTPUT_DIR/graphql_endpoints.txt"

echo "[*] Testing GraphQL with GraphQLmap..."
if [ -s "$OUTPUT_DIR/graphql_endpoints.txt" ]; then
    ...
    echo "[!] No GraphQL endpoints found."
fi

### 15. JWT Token Discovery and Analysis ###
echo "[*] Checking for JWT tokens in responses..."
cat "$OUTPUT_DIR/allurls.txt" | xargs -I % curl -s % | grep -oEi 'eyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+' | tee "$OUTPUT_DIR/jwt_tokens.txt"
